...
import "@ai-protocol/advanced-erc20/contracts/interfaces/ERC1363Spec.sol";
import "@ai-protocol/advanced-erc20/contracts/interfaces/EIP2612.sol";
import "@ai-protocol/advanced-erc20/contracts/interfaces/EIP3009.sol";
...
contract AliERC20v2Base is ERC1363, EIP2612, EIP3009, AccessControl {
	 ...
	 */
	constructor(address _initialHolder, uint256 _initialSupply) AccessControl(msg.sender, 0) {
	...
	function __transferFrom(address _by, address _from, address _to, uint256 _value) private {
		...
		require(
			(_from == _by && isFeatureEnabled(FEATURE_TRANSFERS)) ||
	...
	function permit(
		...
		address signer = __deriveSigner(
			abi.encode(PERMIT_TYPEHASH, _owner, _spender, _value, nonces[_owner]++, _exp),
			v,
			r,
			s
		);
	 ...
	 */
	function authorizationState(address _authorizer, bytes32 _nonce) public view override returns (bool) {
	...
	function transferWithAuthorization(
		...
		address signer = __deriveSigner(
			abi.encode(TRANSFER_WITH_AUTHORIZATION_TYPEHASH, _from, _to, _value, _validAfter, _validBefore, _nonce),
			v,
			r,
			s
		);
	...
	function receiveWithAuthorization(
		...
		address signer = __deriveSigner(
			abi.encode(RECEIVE_WITH_AUTHORIZATION_TYPEHASH, _from, _to, _value, _validAfter, _validBefore, _nonce),
			v,
			r,
			s
		);
	 ...
	 * @param s half of the ECDSA signature pair
	 */
	function cancelAuthorization(address _authorizer, bytes32 _nonce, uint8 v, bytes32 r, bytes32 s) public override {
		// derive signer of the EIP712 ReceiveWithAuthorization message
